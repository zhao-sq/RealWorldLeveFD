...
class RealEnv:
    ...
    def get_obs(self, time_step, to_device=True, from_dataset=False):
        ...
        return {
            "low_obs_dim": state_out,             # float32 (torch if to_device)
            "in_hand": in_hand_rgb,  # 3xHxW float32
            "fixed_left": left_rgb,  # 3xHxW float32
            # "observation.images.fixed_right": right_rgb # 3xHxW float32
...
def get_current_obs(obs_list: dict, n_obs_steps, image_key):
    ...
    obs_dict['low_dim_obs'] = low_dim
    return obs_dict

# =========================
# Rollout loop
# =========================
def run_policy_loop(env: RealEnv, controller: crx_controller, cfg, policy: BaseImagePolicy, keys: KeyWatcher,
                    num_steps=NUM_STEPS, dt=DT, num_episode=1, replay=False, n_obs_steps=2, n_action_steps=8):
...
if __name__ == "__main__":
    ...
    # only load policy parameters
    # policy: DiffusionUnetImagePolicy = hydra.utils.instantiate(cfg.policy)
    # you need to have a ws to have this work, and load very dict inside
    cls = hydra.utils.get_class(cfg._target_)
    workspace: BaseWorkspace = cls(cfg)
    payload = torch.load(open(cfg.ckp_path, 'rb'), pickle_module=dill)
    # cfg = payload['cfg']
